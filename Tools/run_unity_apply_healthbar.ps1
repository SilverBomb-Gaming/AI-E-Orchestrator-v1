...
$defaultArtifactRelative = "scripts\logs\healthbar_applied.json"
$defaultArtifactPath = [System.IO.Path]::GetFullPath((Join-Path $resolvedProject $defaultArtifactRelative))
...
if ($defaultArtifactPath -and ($defaultArtifactPath -ne $resolvedArtifact) -and (Test-Path $defaultArtifactPath)) {
    Remove-Item $defaultArtifactPath -Force
}
...
Write-Host "[run_unity_apply_healthbar] Using Unity editor: $unityExe"
Write-Host "[run_unity_apply_healthbar] Project: $resolvedProject"
Write-Host "[run_unity_apply_healthbar] Scene: $SceneName"
Write-Host "[run_unity_apply_healthbar] Artifact: $resolvedArtifact"
Write-Host "[run_unity_apply_healthbar] Log: $resolvedLog"
...
$arguments = @(
    ...
    "-executeMethod", "Babylon.CI.ApplyHealthBar.Run",
    "-logFile", $resolvedLog,
    "-quit"
)
...
try {
    ...
    Write-Host "[run_unity_apply_healthbar] Unity exited with code $exitCode"

    if (-not (Test-Path $resolvedArtifact) -and $defaultArtifactPath -and (Test-Path $defaultArtifactPath)) {
        try {
            Move-Item -Path $defaultArtifactPath -Destination $resolvedArtifact -Force
            Write-Warning "[run_unity_apply_healthbar] Artifact was emitted to default path '$defaultArtifactRelative'. Moved it to requested location."
        } catch {
            Write-Warning "[run_unity_apply_healthbar] Failed to move artifact from default path: $_"
        }
    }

    $artifactExists = Test-Path $resolvedArtifact
    if (-not $artifactExists) {
        if ($exitCode -ne 0) {
            throw "Unity returned exit code $exitCode and no healthbar artifact was produced at $resolvedArtifact"
        }
        throw "Healthbar artifact not found at $resolvedArtifact"
    }

    $length = (Get-Item $resolvedArtifact).Length
    if ($length -lt $MinArtifactBytes) {
        throw "Healthbar artifact at $resolvedArtifact is too small ($length bytes < $MinArtifactBytes bytes minimum)"
    }
    ...
    try {
        $null = Get-Content $resolvedArtifact -Raw | ConvertFrom-Json
    } catch {
        throw "Healthbar artifact at $resolvedArtifact is not valid JSON: $_"
    }

    if ($exitCode -ne 0) {
        Write-Warning "[run_unity_apply_healthbar] Unity returned exit code $exitCode, but artifact validation passed. Proceeding."
    }

    Write-Host "[run_unity_apply_healthbar] Health HUD artifact captured ($length bytes)."
    exit 0
} catch {
    Write-Error $_
    exit 1
